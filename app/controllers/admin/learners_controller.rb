class Admin::LearnersController < ApplicationController
  ...
  def show
    ...
    @point_today = Point.where(learner_id: @learner.id).where("DATE_FORMAT(created_at, '%Y-%m-%d') = ?", Date.today.strftime('%Y-%m-%d')).count
    @point_this_month = Point.where(learner_id: @learner.id).where("DATE_FORMAT(created_at, '%Y-%m-%d') = ?", Date.today.strftime('%Y-%m')).count
